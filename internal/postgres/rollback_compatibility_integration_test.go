...
func TestSprint08RollbackRunsSprint07BinaryAgainstDowngradedSchema(t *testing.T) {
	binary := os.Getenv("FORJA_TEST_SPRINT07_BINARY")
	if binary == "" {
		t.Skip("FORJA_TEST_SPRINT07_BINARY is not set")
	}
	if info, err := os.Stat(binary); err != nil || info.IsDir() {
		t.Fatalf("Sprint 07 binary is unavailable: info=%v err=%v", info, err)
	}
	pool := integrationPool(t)
	resetDatabase(t, pool)
	if err := Migrate(t.Context(), pool); err != nil {
	...
	t.Cleanup(func() {
		...
		defer cancel()
		if err := Migrate(cleanupContext, pool); err != nil {
			t.Errorf("restore current schema after Sprint 08 rollback rehearsal: %v", err)
		}
	})

	stdout, err := os.CreateTemp(t.TempDir(), "sprint07-stdout-*.log")
	if err != nil {
		t.Fatal(err)
	}
	defer stdout.Close()
	stderr, err := os.CreateTemp(t.TempDir(), "sprint07-stderr-*.log")
	if err != nil {
		t.Fatal(err)
	}
	defer stderr.Close()
	command := exec.Command(
		binary,
		"--listen", "127.0.0.1:0",
		"--environment", "sprint08-rollback-rehearsal",
		"--shutdown-timeout", "2s",
		"--database-auto-migrate=false",
	)
	command.Stdout = stdout
	...
	command.Env = append(
		os.Environ(),
		"FORJA_DATABASE_URL="+integrationDatabaseURL(t),
		"FORJA_HTTP_BEARER_TOKEN=forja-sprint08-rollback-rehearsal-token",
		"FORJA_HTTP_ACTOR_TYPE=system",
		"FORJA_HTTP_ACTOR_ID=sprint08-rollback-rehearsal",
	)
	if err := command.Start(); err != nil {
		t.Fatalf("start Sprint 07 binary: %v", err)
	}
	processDone := make(chan struct{})
	var processErr error
	go func() {
	...
	response, err := client.Get(endpoint + "/readyz") // #nosec G107 -- parsed loopback test endpoint.
	if err != nil {
		t.Fatalf("probe Sprint 07 rollback daemon: %v", err)
	}
	_ = response.Body.Close()
	if response.StatusCode != http.StatusOK {
		t.Fatalf("Sprint 07 readiness status = %d", response.StatusCode)
	}
	if err := command.Process.Signal(syscall.SIGTERM); err != nil {
		t.Fatalf("stop Sprint 07 rollback daemon: %v", err)
	}
	select {
	case <-processDone:
		stopped = true
		if processErr != nil {
			t.Fatalf("Sprint 07 rollback daemon exit: %v", processErr)
		}
	case <-time.After(5 * time.Second):
		t.Fatal("Sprint 07 rollback daemon did not stop within its bounded deadline")
	}
	if err := Migrate(t.Context(), pool); err != nil {
		t.Fatalf("reapply current schema after Sprint 08 rollback rehearsal: %v", err)
	}
	schemaDowngraded = false
}
